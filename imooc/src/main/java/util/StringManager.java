	...
	public static void diff(String s1, String s2) {
		...
		for (int i = 0; i < s1.length() && i < s2.length(); i++)
			if (s1.charAt(i) != s2.charAt(i))
				Debug.print((new StringBuilder("Diff at pos ")).append(i).append(" with {").append(s1.charAt(i))
						.append("} ").append(s1.charAt(i)).append(" and {").append(s2.charAt(i)).append("} ")
						.append(s2.charAt(i)).toString());
